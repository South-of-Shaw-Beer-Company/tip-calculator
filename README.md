...
![Tip Calculator screenshot](docs/screenshot.png)

The calculator is built with Vite, TypeScript, React, and Mantine. It does not use a backend or login. Employee names are remembered in this browser with `localStorage`; hours and tip totals are not.
...
- Remember employee names in this browser so they do not need to be retyped.
...
- Back of House defaults to `50%`.
- Front of House defaults to `50%`.
...
- `src/roster-storage.ts` remembers employee names in `localStorage`.
- `src/styles.css` contains screen and print styles.

## Contributing

See [CONTRIBUTING.md](CONTRIBUTING.md) for setup, checks, and pull request notes.
